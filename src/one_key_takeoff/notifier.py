...
class MissionReporter:
    ...
    async def notify_welcome(self):
        await self.notifier.send_notification(
            self.chat_id,
            "Welcome to Rovonize Systems. Please send a location pin or a Google Maps link to initiate a drone mission.",
        )

    async def notify_extracting_url(self):
        await self.notifier.send_notification(
            self.chat_id, "🔄 Extracting coordinates from Google Maps link..."
        )

    async def notify_invalid_url(self):
        await self.notifier.send_notification(
            self.chat_id,
            "❌ Could not extract coordinates from that link. Please try sending a native WhatsApp location pin.",
        )

    async def notify_url_error(self):
        await self.notifier.send_notification(
            self.chat_id,
            "🚨 Error processing the Google Maps link. Please send a native location pin.",
        )
    ...
    async def notify_geofence_violation(self, distance: float, max_dist: float):
        await self.notifier.send_notification(
            self.chat_id,
            f"❌ Target is {distance:.1f}m away, exceeding the max geofence of {max_dist:.0f}m. Aborting.",
        )
    ...
    async def notify_takeoff(self):
        await self.notifier.send_notification(
            self.chat_id, "🚁 Drone acquired and armed. Taking off..."
        )

    async def notify_arrival(self, distance: float, circle_alt: float):
        await self.notifier.send_notification(
            self.chat_id,
            f"📍 Target reached (within {distance:.1f}m)! Descending to {circle_alt}m and initiating surveillance orbit.",
        )

    async def notify_rtl(self):
        await self.notifier.send_notification(
            self.chat_id,
            "🏠 Surveillance complete. Returning to launch position (RTL).",
        )

    async def notify_error(self):
        await self.notifier.send_notification(
            self.chat_id, "🚨 Mission Error: Triggering emergency fail-safe procedure."
        )
